...
function applyKnowledgeContext(
  ...
  for (let i = cloned.length - 1; i >= 0; i -= 1) {
    ...
    cloned.splice(i, 0, {
      role: "system",
      content: `# テープ式心理学ナレッジ（内部専用）

以下は相談者の状況に関連する深い知識です。最強のツール「心理テスト」や「ピールダウン」の補助として、また解説フェーズで活用してください。

## 活用方法
1. **解説フェーズ（気づきを促す）**で心の仕組みの説明に使う
2. 相談者の状況に応じて最も適切な知識を選ぶ
3. 専門用語（ガムテープ、テープ式心理学）は一般的な言葉（思い込み、心の仕組み）に置き換える
4. 引用や出典は言わず、自然な会話として織り込む
5. 単なる言い換えではなく、深い仕組みを説明する

## 重要な原則
- アドバイスは絶対にしない（相談者が迷子になる）
- 答えは相談者の中にある
- あなたは鏡である
- Why（なぜ）を使わずHow（どう）で掘り下げる

## 参考知識
${knowledgeContext}

**この知識を使って、相談者が自分で気づけるように導いてください。**`,
    });
    break;
  }
